...
const WASM_FILES = [
  ...
  { src: "node_modules/tree-sitter-typescript/tree-sitter-typescript.wasm", name: "tree-sitter-typescript.wasm" },
];
...
for (const { platform, bunTarget, binary } of TARGETS) {
  ...
  console.log(`\nBuilding ${platform}...`);
  execSync(
    `bun build --compile src/cli.ts --target ${bunTarget} --outfile ${binaryPath}`,
    { stdio: "inherit" },
  );
